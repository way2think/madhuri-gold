import { Product } from '@/types';

const diamondProducts: Product[] = [
  {
    id: 'diamond-earrings',
    title: 'Diamond Earrings',
    price: 5000,
    // image: '/assets/images/diamond-necklaces.svg',
    image: '/assets/images/diamond/earring-diamond3.jpeg',
    link: '/diamond',
    category: 'diamond',
    metal: 'diamond',
    productType: 'earrings',
    gender: 'female',
    occasion: 'special',
    createdAt: new Date('2023-01-15T10:00:00Z'),
    breadcrumb: ['Home', 'Diamond', 'Diamond Earring'],
    description:
      'Elegant diamond earrings with diamond accents, perfect for special occasions.',
    specifications: [
      { label: 'Material', value: 'Diamond' },
      { label: 'Weight', value: '10g' },
      { label: 'Purity', value: '18K' },
      { label: 'Style', value: 'Stud' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'ornaments',
    title: 'Ornaments',
    price: 7500,
    // image: '/assets/images/gold-ring.svg',
    image: '/assets/images/diamond/ornaments.jpeg',
    link: '/diamond',
    category: 'diamond',
    metal: 'diamond',
    productType: 'rings',
    gender: 'female',
    occasion: 'wedding',
    createdAt: new Date('2023-02-20T14:30:00Z'),
    breadcrumb: ['Home', 'Diamond', 'Ornaments'],
    description: 'Beautiful diamond-studded ornaments to enhance your beauty.',
    specifications: [
      { label: 'Material', value: 'Gold, Diamond' },
      { label: 'Weight', value: '15g' },
      { label: 'Purity', value: '18K' },
      { label: 'Style', value: 'Traditional' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'diamond-ring',
    title: 'Diamond Ring',
    price: 12000,
    // image: '/assets/images/diamond-ring-removebg-preview 1.svg',
    image: '/assets/images/diamond/diamond-ring.jpg',
    // image: '/assets/images/diamond/earring-diamond1.jpeg',
    link: '/diamond',
    category: 'diamond',
    metal: 'diamond',
    productType: 'rings',
    gender: 'unisex',
    occasion: 'engagement',
    createdAt: new Date('2023-03-10T18:45:00Z'),
    breadcrumb: ['Home', 'Diamond', 'Diamond Ring'],
    description: 'Classic diamond ring with a sparkling solitaire centerpiece.',
    specifications: [
      { label: 'Material', value: 'Gold, Diamond' },
      { label: 'Weight', value: '5g' },
      { label: 'Purity', value: '18K' },
      { label: 'Style', value: 'Solitaire' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'the-prettiest',
    title: 'The Prettiest',
    price: 3000,
    // image: '/assets/images/earings.svg',
    image: '/assets/images/diamond/earring-diamond2.jpeg',
    link: '/diamond',
    category: 'diamond',
    metal: 'diamond',
    productType: 'earrings',
    gender: 'female',
    occasion: 'everyday',
    createdAt: new Date('2023-04-05T09:15:00Z'),
    breadcrumb: ['Home', 'Diamond', 'The Prettiest'],
    description: 'The prettiest pair of diamond earrings for a dazzling look.',
    specifications: [
      { label: 'Material', value: 'Gold, Diamond' },
      { label: 'Weight', value: '8g' },
      { label: 'Purity', value: '18K' },
      { label: 'Style', value: 'Drop' },
    ],
    recommendedProducts: [],
  },
];

const goldProducts: Product[] = [
  {
    id: 'bridal-jewellery',
    title: 'Bridal Jewellery',
    price: 50000,
    image: '/assets/images/gold/earring-gold2.jpeg',
    // image: '/assets/images/bridal.svg',
    link: '/gold',
    category: 'gold',
    metal: 'gold',
    productType: 'necklaces',
    gender: 'female',
    occasion: 'wedding',
    createdAt: new Date('2023-05-12T11:00:00Z'),
    breadcrumb: ['Home', 'Gold', 'Bridal Jewellery'],
    description: 'Traditional gold bridal jewellery set for your special day.',
    specifications: [
      { label: 'Material', value: 'Gold' },
      { label: 'Weight', value: '50g' },
      { label: 'Purity', value: '22K' },
      { label: 'Style', value: 'Bridal Set' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'necklace-set',
    title: 'Necklace Set',
    price: 25000,
    image: '/assets/images/gold/gold-necklace-set.jpg',
    // image: '/assets/images/gold/earring-gold1.jpeg',
    // image: '/assets/images/Carosuelimage.svg',
    link: '/gold',
    category: 'gold',
    metal: 'gold',
    productType: 'necklaces',
    gender: 'female',
    occasion: 'special',
    createdAt: new Date('2023-06-22T16:20:00Z'),
    breadcrumb: ['Home', 'Gold', 'Necklace Set'],
    description: 'Elegant gold necklace set suitable for all occasions.',
    specifications: [
      { label: 'Material', value: 'Gold' },
      { label: 'Weight', value: '30g' },
      { label: 'Purity', value: '22K' },
      { label: 'Style', value: 'Necklace Set' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'gold-bangles',
    title: 'Gold Bangles',
    price: 15000,
    image: '/assets/images/bangleset.svg',
    // image: '/assets/images/gold/earring-gold3.jpeg',
    link: '/gold',
    category: 'gold',
    metal: 'gold',
    productType: 'bangles',
    gender: 'female',
    occasion: 'everyday',
    createdAt: new Date('2023-07-18T13:00:00Z'),
    breadcrumb: ['Home', 'Gold', 'Gold Bangles'],
    description: 'Classic gold bangles with intricate designs.',
    specifications: [
      { label: 'Material', value: 'Gold' },
      { label: 'Weight', value: '20g' },
      { label: 'Purity', value: '22K' },
      { label: 'Style', value: 'Bangle' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'gianna-ring',
    title: 'Gianna Ring',
    price: 8000,
    image: '/assets/images/ringgold.svg',
    // image: '/assets/images/gold/earring-gold4.jpeg',
    link: '/gold',
    category: 'gold',
    metal: 'gold',
    productType: 'rings',
    gender: 'female',
    occasion: 'engagement',
    createdAt: new Date('2023-08-01T10:30:00Z'),
    breadcrumb: ['Home', 'Gold', 'Gianna Ring'],
    description: 'Stylish gold ring with a modern touch.',
    specifications: [
      { label: 'Material', value: 'Gold' },
      { label: 'Weight', value: '6g' },
      { label: 'Purity', value: '22K' },
      { label: 'Style', value: 'Ring' },
    ],
    recommendedProducts: [],
  },
];

const silverProducts: Product[] = [
  {
    id: 'bangles',
    title: 'Bangles',
    price: 4000,
    image: '/assets/images/slivernecklac.svg',
    link: '/silver',
    category: 'silver',
    metal: 'silver',
    productType: 'bangles',
    gender: 'female',
    occasion: 'everyday',
    createdAt: new Date('2023-09-14T12:00:00Z'),
    breadcrumb: ['Home', 'Silver', 'Bangles'],
    description: 'Elegant silver bangles for everyday wear.',
    specifications: [
      { label: 'Material', value: 'Silver' },
      { label: 'Weight', value: '18g' },
      { label: 'Purity', value: '925' },
      { label: 'Style', value: 'Bangle' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'necklace-set-silver',
    title: 'Silver Necklace Set',
    price: 9000,
    image: '/assets/images/necksliver.svg',
    link: '/silver',
    category: 'silver',
    metal: 'silver',
    productType: 'necklaces',
    gender: 'female',
    occasion: 'special',
    createdAt: new Date('2023-10-25T17:00:00Z'),
    breadcrumb: ['Home', 'Silver', 'Necklace Set'],
    description: 'Beautiful silver necklace set with intricate patterns.',
    specifications: [
      { label: 'Material', value: 'Silver' },
      { label: 'Weight', value: '25g' },
      { label: 'Purity', value: '925' },
      { label: 'Style', value: 'Necklace Set' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'silver-earring',
    title: 'Silver Earring',
    price: 2500,
    image: '/assets/images/silver/earring-silver1.jpeg',
    // image: '/assets/images/silver.svg',
    link: '/silver',
    category: 'silver',
    metal: 'silver',
    productType: 'earrings',
    gender: 'female',
    occasion: 'everyday',
    createdAt: new Date('2023-11-30T11:45:00Z'),
    breadcrumb: ['Home', 'Silver', 'Silver Earring'],
    description: 'Chic silver earrings for a modern look.',
    specifications: [
      { label: 'Material', value: 'Silver' },
      { label: 'Weight', value: '4g' },
      { label: 'Purity', value: '925' },
      { label: 'Style', value: 'Stud' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'silver-chain',
    title: 'Silver Chain',
    price: 10000,
    image: '/assets/images/silver-jain.svg',
    link: '/silver',
    category: 'silver',
    metal: 'silver',
    productType: 'necklaces',
    gender: 'female',
    occasion: 'everyday',
    createdAt: new Date('2023-12-15T15:00:00Z'),
    breadcrumb: ['Home', 'Silver', 'Silver Chain'],
    description: 'Classic silver chain with a polished finish.',
    specifications: [
      { label: 'Material', value: 'Silver' },
      { label: 'Weight', value: '10g' },
      { label: 'Purity', value: '925' },
      { label: 'Style', value: 'Chain' },
    ],
    recommendedProducts: [],
  },
  {
    id: 'silver-bangle',
    title: 'Silver Bangle',
    price: 6000,
    image: '/assets/images/silver/silver_bracelet.jpg',
    link: '/silver',
    category: 'silver',
    metal: 'silver',
    productType: 'bangles',
    gender: 'female',
    occasion: 'everyday',
    createdAt: new Date('2023-12-15T15:00:00Z'),
    breadcrumb: ['Home', 'Silver', 'Silver Bangle'],
    description: 'Classic silver bangle with a polished finish.',
    specifications: [
      { label: 'Material', value: 'Silver' },
      { label: 'Weight', value: '10g' },
      { label: 'Purity', value: '925' },
      { label: 'Style', value: 'Bangle' },
    ],
    recommendedProducts: [],
  },
];

const allProducts: Product[] = [
  ...diamondProducts,
  ...goldProducts,
  ...silverProducts,
];

const featuredProducts: Product[] = [
  diamondProducts[0],
  diamondProducts[1],
  goldProducts[0],
  goldProducts[1],
  silverProducts[0],
  silverProducts[1],
];

const productCategories = [
  {
    name: 'Gold',
    products: goldProducts,
  },
  {
    name: 'Silver',
    products: silverProducts,
  },
  {
    name: 'Diamond',
    products: diamondProducts,
  },
];

export {
  diamondProducts,
  goldProducts,
  silverProducts,
  allProducts,
  productCategories,
  featuredProducts,
};
